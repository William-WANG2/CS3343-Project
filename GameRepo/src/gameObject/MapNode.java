...
public class MapNode implements FrameUpdate {
	...
	public void update(Mouse mouse, Key key) {
		boolean isInGeo = Math.pow(mouse.mousePos.x - info.displayPos.y - info.radius, 2)
				+ Math.pow(mouse.mousePos.y - info.displayPos.x - info.radius, 2) < Math.pow(info.radius, 2);
				...
				if (updateNode == null || updateNode == this) {
					...
					if (updateNode == this) {
						if (Board.isInputValid()) {
							info.blocked = true;
							Mythread thread = new Mythread("res/textures/good.wav");
							thread.start();
	...
	public void render(Graphics2D g) {
		...
		if (info.blocked) {
			transform = new AffineTransform(hole.getScaleX(), 0.0, 0.0, hole.getScaleY(), info.displayPos.y,
					info.displayPos.x);
