...
public class Map implements FrameUpdate{
	...
	public void initialize(int m, int n, int width, int height) {
		map = new MapNode[m][n];
		dummyNode = new MapNode(0, 0, 0, 0, 0);
		...
		for(int i=0; i<m; i++) {
			for(int j=0; j<n; j++) {
				//set x, y coordinates
				if(i%2 == 0) {
					map[i][j] = new MapNode(sY+interval+2*i*(radius+interval), sX+interval+2*j*(radius+interval), radius, i, j);
				}
				else {
					map[i][j] = new MapNode(sY+interval+2*i*(radius+interval), sX+interval+2*j*(radius+interval)+offset, radius, i, j); //with offset to the right
				}
				//set adjacent nodes
				if(i%2 == 0) {
					if(i-1>=0) {
