...
public class Dio {
	...
	public void update(Graphics2D g) {
		...
		Vector2d dir=s.computeDecision(info.getDisplaypos());
		if (isAlive()) {
			if(dir==null) {
				setSurround(true);
				moveSurround();
				g.drawImage(angrydio, info.getDisplaypos().y,  info.getDisplaypos().x, 50, 50, null);
			}
			else {
				moveNormal(dir);
				g.drawImage(dio, info.getDisplaypos().y,  info.getDisplaypos().x, 50, 50, null);
			}
